...
RSpec.describe Api::ScormCoursesController, type: :controller do
  ...
  before(:example) do
    ...
    expect(controller).to receive(:scorm_cloud_service).at_most(3).times.and_return(@mock_scorm)
  end
  ...
  describe "POST create" do
    it "should upload scorm package" do
      expect(@mock_scorm).to receive(:upload_course).with("fake_file", "course_id")
  ...
  describe "GET preview" do
    it "should return preview url" do
      ...
      expect(response).to have_http_status 200
      expect(JSON.parse(response.body)["response"]).to eq({"launch_url" => "fake_url"})
    end
  end
end
