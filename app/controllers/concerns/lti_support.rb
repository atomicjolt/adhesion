module Concerns
  module LtiSupport
    ...
    def generate_user(params)
      ...
      name = if params[:lis_person_name_full]
               params[:lis_person_name_full]
             else
               lis_person = params[:lis_person_name_given]
               lis_family = params[:lis_person_name_family]
               "#{lis_person} #{lis_family}"
             end
      name = name.strip
      name = params[:roles] if name.blank?
      lms_roles = params[:roles]

      # If there isn't an email then we have to make one up.
      # We use the user_id and instance guid
      domain = params["custom_canvas_api_domain"] ||
        Rails.application.secrets.application_url
      email = params[:lis_person_contact_email_primary]
      email = "user-#{params[:user_id]}@#{domain}" if email.blank? &&
      ...
      user.lti_provider          = lti_provider
      user.lms_user_id           = params[:custom_canvas_user_id] ||
        params[:user_id]
      user.skip_confirmation!
      lms_roles.split(",").each do |lms_role|
        user.add_to_role(lms_role)
      end
    ...
    def user_from_lti
      ...
      if user.blank?
        generate_user(params)
      else
        user.roles = []
        params[:roles].split(",").each do |lms_role|
          user.add_to_role(lms_role)
        end
      end
      user
